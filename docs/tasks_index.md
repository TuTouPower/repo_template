...
- `backlog` 已建目录，含已填写的 `spec.md` / `plan.md` / `task.md`（验收标准非空；front matter 含大写 `tid`/`slug`）；`active` 补齐 owner / branch，且必须在 `task_{TID}_{slug}` 分支上工作。
- `done` 及所有 `dropped` 任务目录**一律**移入 `docs/archive/tasks/`。
- owner 和 branch 表示当前归属；工作分支推荐 `task_{TID}_{slug}`（如 `task_T001_foo`）。
