...
- 非归档 Markdown 统一用 md_kx 格式化（`scripts/repo_template/md_format.py`），表用 `compact`（`|a|b|`）。格式由 `.md_kx.toml` 统一，禁止 prettier / 按列 pad。commit 由 pre-commit hook 强制（`scripts/repo_template/hooks/pre-commit`，格式化本次 staged 的 `.md` 并重新暂存；工作区与 index 不一致则拒绝），需先 `python3 scripts/repo_template/repo_sync.py install-hooks` 启用 `core.hooksPath`（已有其它 hooksPath 须 `--force`）；临时手动格式化用 `python3 scripts/repo_template/md_format.py --changed`，commit 前 `--check` 为绿。
