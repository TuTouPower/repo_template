...
- 总账编号：`docs/pending.md` 的未修 bug 与遗留待办统一使用 `pNNN`；新增条目前用 `scripts/pending.py next` 扫描当前与归档总账，两个业务小节共享全局序列，历史编号不复用；不维护 pending index。`docs/findings.md` 用 `dNNN`，递增不复用。
