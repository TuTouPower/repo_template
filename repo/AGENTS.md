...
命名与格式约定见 `.repo_template/docs/conventions.md`「命名与格式」。项目级约定见 `docs/blueprint/conventions.md`。
...
实施阶段只写 worktree，调度合并阶段写主仓；attempt 生命周期与合并授权细节见 `.repo_template/docs/architecture.md`。
...
### `.repo_template/scripts/task.py` 使用示例
