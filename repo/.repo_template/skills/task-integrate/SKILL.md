...
```bash
.repo_template/scripts/task.py cleanup-worktree {tid} \
...
```bash
.repo_template/scripts/task.py integrate {tid} \
  --attempt {N} --execution-id {EXECUTION_ID}
```
...
```bash
.repo_template/scripts/task.py integrate-chain {TAIL_TID}
```
...
```bash
.repo_template/scripts/task.py integrate-chain {TAIL_TID} --continue
```
