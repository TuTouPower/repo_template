...
把 **repo_template** 的工具链与模板资产同步到**当前消费项目**。本 skill 由 `repo-template-sync` 启动器每次同步前从模板源硬同步保证最新；**本 skill 不负责启动 / 自更新**。直接调用本 skill 前，先跑 `repo-template-sync` 确保其为最新版。禁止在模板仓本身当「推送源」跑。

**机械化对齐一律由脚本 `.repo_template/scripts/repo_sync.py` 执行**——能用脚本做的不手动拼命令。agent 只负责：裁定非机械化单元（共享文稿语义合并）、审查预览、执行审批门禁、管理站立指令登记。
...
仅由 `repo-template-sync` 启动器委派执行；用户显式请求同步时先经启动器。用户不传参数；agent 全自动按「流程」执行。`init` / `status` / `plan` / `apply` 是**流程内部阶段**（脚本子命令），不是用户可敲的命令。
...
- **不静默硬盖可定制的共享资产**：`AGENTS.md`、`.gitignore`、MCP、宿主 settings 等必须走「裁定」，逐项裁定。模板资产（整包 `.repo_template/`）归「强制覆盖」。项目级 `docs/blueprint/conventions.md` 不参与同步。
- **合并共享文稿先禁 prettier/pad，语义合并后用 md_kx 收尾**：合并 `AGENTS.md` 时禁止 prettier / 按列 pad；语义合并结束后用 `.repo_template/scripts/md_format.py` 做最终格式化。
...
`repo_sync.py` 在消费项目 `.repo_template/scripts/` 下（硬同步清单路径，随模板演进同步）。子命令见 `repo_sync.py --help`。

|能力|命令|说明|
|---|---|---|
|初始化 template_source|`repo_sync.py init --source <path\|url>`|首次接入；仅字段级写 state|
|同步工具自举|`repo_sync.py prep`|由启动器调用：单向覆盖 core skill 与 `.repo_template/scripts/`，建软链；不写 state、不做删除|
...
1. 本 skill 须已由 `repo-template-sync` 启动器硬同步为最新；直接调用前先跑启动器。
2. `sync_state.json` 须已有 `template_source`；缺失 → 提示先跑 `repo-template-sync`（或 `repo_sync.py init --source <path|url>`），**停止并报告**。
...
- **裁定同步**：`AGENTS.md`、`.gitignore` 的分类（`template_only` / `consumer_only` / `both_identical` / `both_differ`）与建议 disposition。**有 diff 的单元必出现**。
- **软链**状态与需修项。
...
对 `AGENTS.md` 等语义合并单元，agent 依据 plan 的分类**读 diff 后裁定**：
...
2. 组装裁定决策，调用：
    ...
    ```bash
    python3 .repo_template/scripts/repo_sync.py apply \
      --decision AGENTS.md:update \
      # ... 仅已确认的单元；未确认不传
    ```

    apply 执行：硬同步树对树 → skill 整目录覆盖 → 软链建/修 → .gitignore/MCP 机械合并 → 裁定单元按决策写盘（`merge` 由 agent 先前手动编辑，脚本跳过）→ `pytest .repo_template/tests/ -q`（`--skip-tests` 可跳过）→ 字段级更新 state（干净 SRC + 测试通过才推进 `last_synced_commit`）→ 输出**改动路径清单**。
...
2. 用 apply 输出的**改动路径清单**（并上启动器写盘路径）列出**拟提交路径**（仅本轮同步触碰的路径，含变更的 `sync_state.json`）。
