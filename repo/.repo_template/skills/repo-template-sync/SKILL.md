...
1. **模板源就绪**：
    - 消费侧无 `.repo_template/scripts/repo_sync.py` → 先从模板源复制该脚本（首次接入引导；完整工具链由步骤 2 补齐）。
    - 无 `.repo_template/sync_state.json` → `repo_sync.py init --source <path|url>`。推断模板源：用户给定 → 常见本机路径 → 不臆造。含 `.repo_template/scripts/task.py` 或工厂根下 `repo/.repo_template/scripts/task.py` → `kind=path`；否则 `url`。
    - 已有 state → 读 `template_source`。缺源且无法初始化 → **停止并报告**。
2. **刷新同步工具（脚本执行）**：`python3 .repo_template/scripts/repo_sync.py prep` 从模板源**单向覆盖**（新增 + 覆盖，不做删除）`.repo_template/skills/repo-template-sync-core/` 与 `.repo_template/scripts/`，并建软链 `.claude/skills/repo-template-sync-core`。模板为唯一真相，覆盖即写盘、无裁定；模板源为 url → `resolve_src` 内部先 clone/更新本地缓存。
3. **重读**消费侧 `.repo_template/skills/repo-template-sync-core/SKILL.md`，以新版本为流程定义与边界。
