...
|L5|审阅对所有 task 无差别|feedback §3/C、retro_t041_t061 §2、retro_t001_t007 痛点 3|`review_level: full\|single\|none` 进 front matter，判不准取 `full`|`task.py add --review-level`、`task-run` Step 5|已落地|
...
|L11|TDD 顺序违规：改测试适配实现|analysis §3|旧绿测只许保留或整体删除并说明；禁止就地改预期；reviewer 必须复核改测方向|AGENTS「开发原则」、`task-run` Step 3、`test_prompt.txt`|已落地|
...
|L14|「一 task 一 commit」与实战脱节|feedback B、retro_t041_t061 §7|**保留「一 task 一 commit」**：六份 review 主张放开多 commit，但与黑盒/审阅质量门禁矛盾（回头改要么 amend 破坏历史，要么新 commit 污染历史）。task 太大就回 `task-create` 拆，不在执行期切分 commit。commit 须可独立验证、有工程意义|`AGENTS.md`「Task 工作流入口」表 + task-run Step 7c|已落地|
|L15|大模型进 plan mode 不执行既定 plan|11111#2（已并入本表）|用户触发 `task-run` 即视为队列已批准；禁止 plan mode、禁止开跑前二次征求同意|AGENTS「硬约束」、`task-run` 开头|已落地|
...
|L21|依赖关系散落 spec 文字，无法机器校验；后续批次重复依赖 Agent 分析|feedback D、retro_t041_t061、review_task_batch_scheduling 多路审阅|task front matter 增加 `depends_on`、`conflicts_with`、`schedule_status`；`task-schedule` 负责 Agent 分析落盘，`task.py plan` 负责后续纯脚本调度（自原 `next-batch --done` 演化）。只做调度层，不修改 `task-run` / `task.py start` / 执行与合并流程|`scripts/task.py`、`task-schedule`、task 模板|已落地|
|L22|subagent prompt 内联正文撑爆 context|analysis §2|派发只传文件路径，正文写 `.scratch/review_prompts/`|`task-run` Step 5|已落地|
|L23|遗留待办与 bugs 无统一登记|11111#1、analysis §13|`bugs.md` 扩为 `docs/pending.md`，「未修 bug」(`bNNN`) 与「遗留待办」(`fNNN`) 两节共一个入口；遗留 finding 的唯一落点是该节，`task.md` 只留 `fix_ref` 引用|`docs/pending.md`、`task-run` Step 6/7、`task.md` 模板|已落地|
...
|L34|调度控制面边沿触发：worker 全灭无人察觉、integrate 后忘补位|omni_media 两起事故（2026-08）、用户提出、多路 diff 审阅|边沿触发 → 水位触发：`task.py reconcile` 幂等 diff 唯一动作来源 + 空闲许可；调度账本（`docs/runtime/dispatch_ledger.jsonl`）记 attempt；worker 交接契约 handoff.json 机器验证；失败分类策略表 + 模型阶梯 + cron 兜底|`plans/plan_dispatch_control_plane.md`、`task.py`、task-dispatch/task-work/task-integrate skill|已落地；被 L35 取代|
|L35|放弃 dispatch 自动并发，并发只留用户手动|用户决定（2026-08-06）|`task-dispatch` 退役：并发不再由 coordinator 自动扇出，只允许用户手动并发——`task-schedule` 落盘后，用户在多个会话各自跑 `task-run`；`task.py view --serve` 起本地只读看板呈现依赖/冲突/运行中状态（Mermaid DAG，端口 0 自分配，从 WSL 调 Windows 默认浏览器）。`start` 加调度门：`depends_on` 未完成（完成口径=`done`/`dropped`，不要求已合并主干）硬拒，最新前置未合并主干时 base 自动落到其分支 tip；`conflicts_with` 对方正在运行（登记 worktree 存在 且 `status=active`）只警告后放行。同时退役 dispatch 配套 API：`attempt bind / escalate / silent-alert`、`observe`、`reconcile` 子命令与对应函数删除；attempt 状态机移除 `reserved/bound/escalated/observation/silent_alerted` 事件类型，executor 收为仅 `inline`；worker/coordinator 角色术语去除，改为「实施阶段 / 调度合并阶段」中性表述。`compute_ps_rows`、`verify_integrate_ready`、ledger、handoff.json、merge_guard 保留（断点恢复依据）。理由：① dispatch 实战效果差，小问题频发、执行不顺畅；② 编程 Agent 能力快速演进，工作流自造特殊机制（hook、skill、控制面）即使暂时有效，也终将被 Agent 自身能力消化——方向错误，工作流应保持薄而通用|`plans/plan_manual_concurrency.md`、task-dispatch skill（已删）、`task.py view --serve`（`view_server.py`）、`cmd_start` 调度门、`architecture_repo_template.md`、CLAUDE.md/AGENTS.md 工作流表与目录约定、`attempts.py` / `monitoring.py` / `control.py` / `cli.py` / `context.py` / `task.py`|已落地|
