...
| L5 | 审阅对所有 task 无差别 | feedback §3/C、retro_t041_t061 §2、retro_t001_t007 痛点 3 | `review_level: full\|single\|none` 进 front matter，判不准取 `full` | `task.py add --review-level`、`tasks-run` Step 5 | 已落地 |
...
| L14 | 「一 task 一 commit」与实战脱节 | feedback B、retro_t041_t061 §7 | **保留「一 task 一 commit」**：六份 review 主张放开多 commit，但与黑盒/审阅质量门禁矛盾（回头改要么 amend 破坏历史，要么新 commit 污染历史）。task 太大就回 `task-create` 拆，不在执行期切分 commit。commit 须可独立验证、有工程意义 | `AGENTS.md`「Task 工作流入口」表 + tasks-run Step 7c | 已落地 |
