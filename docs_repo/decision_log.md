...
| L35 | 放弃 dispatch 自动并发，并发只留用户手动 | 用户决定（2026-08-06） | `task-dispatch` 退役：并发不再由 coordinator 自动扇出，只允许用户手动并发——`task-schedule` 落盘后，用户在多个会话各自跑 `task-run`；`task.py view --serve` 起本地只读看板呈现依赖/冲突/运行中状态（Mermaid DAG，端口 0 自分配，从 WSL 调 Windows 默认浏览器）。`start` 加调度门：`depends_on` 未完成（完成口径=`done`/`dropped`，不要求已合并主干）硬拒，最新前置未合并主干时 base 自动落到其分支 tip；`conflicts_with` 对方正在运行（登记 worktree 存在 且 `status=active`）只警告后放行。同时退役 dispatch 配套 API：`attempt bind / escalate / silent-alert`、`observe`、`reconcile` 子命令与对应函数删除；attempt 状态机移除 `reserved/bound/escalated/observation/silent_alerted` 事件类型，executor 收为仅 `inline`；worker/coordinator 角色术语去除，改为「实施阶段 / 调度合并阶段」中性表述。`compute_ps_rows`、`verify_integrate_ready`、ledger、handoff.json、merge_guard 保留（断点恢复依据）。理由：① dispatch 实战效果差，小问题频发、执行不顺畅；② 编程 Agent 能力快速演进，工作流自造特殊机制（hook、skill、控制面）即使暂时有效，也终将被 Agent 自身能力消化——方向错误，工作流应保持薄而通用 | `plan_manual_concurrency.md`、task-dispatch skill（已删）、`task.py view --serve`（`view_server.py`）、`cmd_start` 调度门、`architecture_repo_template.md`、CLAUDE.md/AGENTS.md 工作流表与目录约定、`attempts.py` / `monitoring.py` / `control.py` / `cli.py` / `context.py` / `task.py` | 已落地 |
