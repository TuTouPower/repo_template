...
> **已落地，本文设计已被 `task.py plan` 取代（2026-08-12 核对；2026-09-07 勘误）**：调度图落 task front matter（`depends_on` / `conflicts_with`）+ `task.py plan` 推本波链（decision_log L21）。`schedule_status` 与 `next-batch` 已由 L36 删除。正文是当时方案推导，不是现行调度手册。
...
1. 后续用户把已完成 task ID 传给固定脚本，脚本用纯图算法计算下一批，零 LLM。
1. 只设计调度层。脚本输出 task ID 后，用户自行调用多个 Agent 分别执行现有 `tasks-run tNNN`；不改 `tasks-run`、`task.py start`、worktree、分支或合并流程。
...
1. 读取 backlog task 的 spec 范围、非范围、依赖与约束、blueprint 更新点。
1. 推导代码路径、共享契约和显式顺序依赖。
1. 生成 `depends_on` DAG 和 `conflicts_with` 无向图，校验悬空引用与环。
1. 对每个分析范围内 task 写入：
   - 判断完整：依赖/冲突字段 + `schedule_status=scheduled`；
   - 无法判断：`schedule_status=pending_clarification`，不猜测。
1. 调用 `task.py next-batch` 输出第一批。
...
1. 未合并 task 分支 ref 的累计状态；
1. main 状态；
1. `docs/archive/tasks/` 中的 done/dropped 状态。
...
1. 去除可选的大小写 `t` 前缀；
1. 剩余部分必须是正整数数字串；
1. 转为整数后，在当前 task、归档 task、worktree/ref task 全集中按数值唯一匹配；
1. 输出和内部计算使用仓库实际记录的规范 tid。
...
1. DAG：无依赖、链式、菱形、环、悬空引用、引用 dropped task。
1. 冲突图：候选间冲突、与 active/blocked 冲突、单向旧数据归一化、贪心顺序稳定。
1. 状态来源：主仓、登记 worktree、未合并 ref、archive 混合状态遵循优先级。
1. `--done`：逐一覆盖 `t11 t012 13 t0015 T14 T00025`、逗号/空格混合、未来四位规范 tid、无匹配和非法输入。
1. assumed done：参数覆盖仓库 active/blocked 后正确解锁下游，且磁盘状态不变。
1. edit：依赖/冲突覆盖、追加、移除、清空、去重、反向边同步和不可编辑引用拒绝。
1. 生命周期：add、rewind、tasks-merge、drop、finish 后调度状态符合表格。
1. 输出：固定段落、空段省略、错误前缀稳定；不依赖 Agent。
1. 回归：无调度字段的旧 task 仍可由既有 `tasks-run` 正常执行；`task.py start` 行为不变。

## 实施拆分

1. **调度字段与生命周期**：front matter、edit、对称边、add/rewind/merge/drop 规则。
1. **`next-batch` 脚本**（依赖 1）：状态发现、图校验、算法、`--done` 人机解析和固定输出。
1. **`tasks-schedule` skill 迁移**（依赖 2）：改名、软链、路由、分析落盘和首批输出。
