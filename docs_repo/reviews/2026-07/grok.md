...
1. 当前 branch 匹配 tid
1. 非独占仓库 → 创建 worktree 再改代码
1. 禁止在共享目录长期 uncommitted 实现
...
1. spec 增加：未知契约清单、可测试性 / `[deploy]` AC、有意不测。
1. TDD 硬句：旧绿测只删不改预期；改测必须新红测覆盖新语义。
1. blocked 增加 `--reason infra`。

### P2

8. `depends_on` 进 index 或 task front matter。
1. 遗留统一登记（bugs 或 followups）。
1. `known_pitfalls.md` 模板（可选）。
...
1. `workflow_retrospective.md`（merge / 分级）
1. `workflow_feedback.md` 后半「按读者切」（文档模型）
1. `workflow_record.md`（隔离事故）
1. `workflow_retrospective_0.md`（历史噪音分类，可选）
1. `archive/workflow_skill_split_proposal.md`（已落地背景，可选）
...
1. 裁决 §3.3 四个冲突；
1. 执行 §6 的 P0；
1. 给 `docs_repo` 加索引与「落地状态」列；
1. 消化 `11111.md`，避免碎片继续增生。
