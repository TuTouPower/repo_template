...
1. **立即**：`test_cmd` 加 `build` + `lint`（方案 A，已实施于 omni_media）
1. **建 task**：修 typecheck:test 存量错误（omni_media 已登记 f043，important）
1. **评估**：lint 存量是否走基线快照（方案 B）还是一次性清理（t084 先例：曾清 4828 个，但回潮）
1. **评估**：worktree build 环境修复（方案 C）是否进模板
