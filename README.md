...
6. 安装 md_kx（Markdown 格式化，见 `scripts/repo_template/md_format.py`）：`uv tool install md_kx`，确认 `md_kx --table-mode` 支持 `compact`/`spaced`/`pad`（旧版不支持则先升级 md_kx 仓 t008/p008）。`.md_kx.toml` 已随模板硬同步。
7. 填写 `docs/blueprint/architecture.md`、`domain.md`、`conventions.md`、`testing.md` 初稿；`decisions.md` 初始可为空。
8. 确认 `docs/tasks/` 下无遗留 task 目录、`docs/specs_index.md` 无伪 active 数据；`docs/tasks/task_template/`（`spec.md` / `task.md`）仅作复制源。
9. 保留 `tests/repo_template/` 下模板自带的测试文件——它们持续验证 task 工具链（`scripts/repo_template/task.py`、完整的 `scripts/repo_template/repo_task/` 实现包、`scripts/repo_template/pending.py`、`scripts/repo_template/spikes.py` 等）的正确性。`task.py` 是 façade，不能脱离 `repo_task/` 单独复制；上述 `rsync` 会保留整套工具链。
