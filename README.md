...
8. 保留 `tests/repo_template/` 下模板自带的测试文件——它们持续验证 task 工具链（`scripts/repo_template/task.py`、完整的 `scripts/repo_template/repo_task/` 实现包、`scripts/repo_template/pending.py`、`scripts/repo_template/spikes.py` 等）的正确性。`task.py` 是 façade，不能脱离 `repo_task/` 单独复制；上述 `rsync` 会保留整套工具链。
