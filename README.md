...
2. 可选：删除 `docs_repo/`（仅本模板仓库的设计笔记，与业务 `docs/` 无关）。
3. 初始化版本控制。
4. 全局替换 `{project_name}`，填写项目一句话介绍和 `AGENTS.md` 硬约束（含 `{doctor_cmd}` / `{test_cmd}` / `{blackbox_cmd}`）。
5. 保持全部软链：`CLAUDE.md -> AGENTS.md`，以及 `.claude/skills/* -> ../../.agents/skills/*`。
6. 按技术栈补充依赖文件、工具配置和 `.gitignore`。
7. 填写 `docs/blueprint/architecture.md`、`domain.md`、`conventions.md` 初稿；`decisions.md` 初始可为空。
8. 确认 `docs/tasks_index.json`、`docs/specs_index.md` 无伪 active 数据；`docs/tasks/task_template/` 仅作复制源。
