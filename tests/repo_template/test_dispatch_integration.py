...
def test_observe_appends_first_and_changed_only(git_repo):
    identity = _reserve(git_repo, "t001", executor="agent", model="opus")
    started = _task_cli(git_repo, "start", "t001")
    assert started.returncode == 0, started.stderr
    bound = _task_cli(
        git_repo, "attempt", "bind", "t001", *_identity_args(identity),
        "--host-worker-id", "host-7",
    )
    assert bound.returncode == 0, bound.stderr

    first = _task_cli(git_repo, "observe", "t001", *_identity_args(identity), "--json")
    second = _task_cli(git_repo, "observe", "t001", *_identity_args(identity), "--json")
    assert first.returncode == second.returncode == 0
    assert json.loads(first.stdout)["changed"] is True
    assert json.loads(second.stdout)["changed"] is False
    assert json.loads(first.stdout)["execution_id"] == identity["execution_id"]
    assert json.loads(first.stdout)["host_worker_id"] == "host-7"
    observations = [event for event in _read_ledger(git_repo) if event["event"] == "observation"]
    assert len(observations) == 1
    assert observations[0]["execution_id"] == identity["execution_id"]

    (_worktree(git_repo, "t001") / "new.bin").write_bytes(b"\x00\xffchanged")
    changed = _task_cli(git_repo, "observe", "t001", *_identity_args(identity), "--json")
...
def test_observe_rejects_unknown_attempt_mismatched_owner_and_missing_worktree(git_repo):
    identity = _reserve(git_repo, "t001", executor="agent")
    started = _task_cli(git_repo, "start", "t001")
    assert started.returncode == 0, started.stderr
    bound = _task_cli(
        git_repo, "attempt", "bind", "t001", *_identity_args(identity),
        "--host-worker-id", "host-8",
    )
    assert bound.returncode == 0, bound.stderr

    unknown = _task_cli(
        git_repo, "observe", "t001", "--attempt", str(identity["attempt"] + 1),
        "--execution-id", "unknown",
    )
    assert unknown.returncode != 0
    assert "未 reserve" in unknown.stderr

    wrong_identity = _task_cli(
        git_repo, "observe", "t001", "--attempt", str(identity["attempt"]),
        "--execution-id", "wrong",
    )
    assert wrong_identity.returncode != 0
    assert "未 reserve" in wrong_identity.stderr or "不是当前 identity" in wrong_identity.stderr

    worktree = _worktree(git_repo, "t001")
    _git(worktree, "checkout", "-b", "t999_wrong")
    mismatch = _task_cli(git_repo, "observe", "t001", *_identity_args(identity))
    assert mismatch.returncode != 0
    assert "归属不符" in mismatch.stderr

    _git(worktree, "checkout", "t001_alpha")
    _git(git_repo, "worktree", "remove", "--force", str(worktree))
    missing = _task_cli(git_repo, "observe", "t001", *_identity_args(identity))
    assert missing.returncode != 0
    assert "worktree 不存在或未登记" in missing.stderr


# --------------------------------------------------------------------------
# start / cleanup / integrate：真实 Git topology、terminal gate 与 MERGE_HEAD
# --------------------------------------------------------------------------
