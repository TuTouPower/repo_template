...
用法：
  python3 scripts/repo_template/md_format.py PATH...      # 点名路径（须在仓库内）
  python3 scripts/repo_template/md_format.py --changed    # 相对 HEAD 的改动 + staged + 未跟踪 .md
  python3 scripts/repo_template/md_format.py --all        # 全部已跟踪非黑名单 .md
  python3 scripts/repo_template/md_format.py --check      # 只检查不写入，格式漂移非 0
