...
1. **读总账**（只读）：
   ...
   - 补扫尚未登记进总账的项（登记本应由 `task-run` 收尾完成，此处扫漏）：
     ...
     - 用户点名的目录或 diff
   - 扫到未登记项：先运行 `scripts/repo_template/pending.py next` 取编号，补进 `docs/pending.md`（按普通或 bug 模板选；`- 来源` 写 finding_id 或原 tid），再走下面流程。总账是唯一入口，不绕过。
...
7. **询问提交总账回写**。`task-create` 已批量提交 task 目录与派生 index；这里只列出 `docs/pending.md`、`docs/archive/pending.md`，询问用户是否提交。
