...
```text
准备串行执行：{tid 列表}（链式）
每个 task 完成后仅 cleanup-worktree，不合并；链全部完成后一次性 merge --no-ff 链尾进本地 {主干} → 重建 index → 合并后验证 → 删除整条链分支
不 push，不动远程。
...
每个 task 从上一个已完成 task 的分支创建（`--base`），因此自动继承前一个 task 的成果。`depends_on` 边必须排在被依赖者之后。
...
2. 未合并 task 分支：用 `scripts/repo_template/task.py show <tid> --ref <branch>` 读分支中状态；已 `done` 则记录其分支为下一个 `--base`，跳到队列下一个 backlog。
...
汇报：固定队列、已完成 tid、链尾分支与 HEAD、main 是否已合并链尾、index 维护结果、删除的链分支、停止原因与剩余队列（若有）、遗留 worktree 与分支（若有）。
