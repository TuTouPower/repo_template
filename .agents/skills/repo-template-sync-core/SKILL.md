...
|同步工具自举|`repo_sync.py prep`|由启动器调用：单向覆盖 core skill 与 scripts/repo_template，建软链；不写 state、不做删除|
...
5. 审批通过后——**只点名 add 清单内路径**（**禁止** `git add -A` / `git add .`）：
    ...
    git add -- scripts/repo_template \
      ...
      .agents/skills/repo-template-sync \
      .agents/skills/repo-template-sync-core \
      .claude/skills/repo-template-sync-core \
      .agents/skills/repo-template-sync/sync_state.json \
      # …apply 输出的每一改动路径
