...
3. **每个 task 落盘**：
   ...
   3. 只读仓库，填写 `spec.md`：
      ...
      - 契约区在 `start` 时锁 hash，执行期不再改；上下文区执行期可补。
...
4. **自检**：AC 可验收；`spec.md` / `task.md` 无残留 `{...}` 占位符（`task.py preflight` 会拦，提前清掉省一轮）。
...
- 不 `start` / `finish` / 实施修复。
- 不写实现或测试到未 ignore 路径；编码与 spike 需求写进 spec 上下文区，等 `tasks-run`。
- 新建时只读仓库。task 状态只经 `scripts/task.py`（状态权威 = `task.md` front matter，两个 index JSON 是派生缓存，已 gitignore，不入 commit）。
- 未经用户同意不 commit 创建物。
